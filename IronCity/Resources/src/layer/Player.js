...
({
    ...
    //     cc.log("----In touch one by one.");
    //     return true;
    // },
    onTouchBegan:function (touches, event) {
        this.m_tBeginPos = touches.getLocation();

        var touchPos  = touches.getLocation();
        var setBtnPos = GameScene.getScene().menuLayer.settingBtn.getPosition();
        var _scale = 640/VisibleRect.getVisibleRect().size.height;
        setBtnPos.x /= _scale;
        setBtnPos.y /= _scale;
        var setBtnSize = GameScene.getScene().menuLayer.settingBtn.getSize();
        //if click the set button, will response it's click event.
        if (!this.checkIfTouchNotInSetBtnArea(touchPos, setBtnSize, setBtnPos)){
            return false;
        }
        return true;
    },
